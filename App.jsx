...
const ident = (f) =>
  f.registration || f.test_registration || (f.msn ? `MSN ${f.msn}` : "Unassigned");
      ...
      sock.onmessage = (m) => {
        ...
        if (d.type === "milestone") {
          setFeed((f) => [{
            ...
            frame: d.registration || d.hex, type: d.type, stage: d.stage,
  ...
  return (
    <div className="wrap">
      <header className="head">
        <div>
          <h1>Riyadh Air<br />Delivery Watch</h1>
          <p className="sub">{roster.length || 158} firm airframes · Boeing + Airbus</p>
        </div>
      ...
      <div className="cols">
        <section>
          <div className="chips">
            {["ALL", "BOEING", "AIRBUS", "787-9", "787-10", "A350-1000", "A321neo"].map((k) => (
              <button key={k} className={`chip${filter === k ? " on" : ""}`} onClick={() => setFilter(k)}>
                {k === "ALL" ? "All" : k === "BOEING" ? "Boeing" : k === "AIRBUS" ? "Airbus" : k}
              </button>
            ))}
          </div>
          <div className="grid">
