...
export function createDb(pool) {
  return {
    ...
    async createCandidate(c) {
      await pool.query(
        `INSERT INTO candidate (hex, registration, type_code, site_icao, first_seen)
        ...
        [c.hex, c.reg, c.type, c.site, c.first_seen]);
    },
    ...
    async roster() {
      const { rows } = await pool.query(
        `SELECT id, manufacturer, type_code, msn, line_number, registration,
                test_registration, icao_hex, current_stage, updated_at
           FROM airframe
    ...
    async recentEvents(limit = 60) {
      const { rows } = await pool.query(
        `SELECT e.id, e.stage, e.occurred_at, e.source, e.site_icao,
                ...
                a.test_registration, a.msn, a.manufacturer
